...
int main(int argc, char *argv[])
{
    ...
    fclose(fptr);
#ifdef DEBUG
    for (int i = 0; i < num_inlet; i++)
        printf(" centroid of inlet %i : %lf %lf %lf\n", i, cen_inlet[3 * i], cen_inlet[3 * i + 1], cen_inlet[3 * i + 2]);
#endif
    ...
    for (int i = 0; i < (3 * num_inlet); i++)
        cen_all_holes[i] = cen_inlet[i];
    for (int i = 0; i < (3 * num_outlet); i++)
        cen_all_holes[3 * num_inlet + i] = cen_outlet[i];
#ifdef DEBUG
    for (int i = 0; i < num_outlet; i++)
        printf("- centroid of outlet %i : %lf %lf %lf\n", i, cen_outlet[3 * i], cen_outlet[3 * i + 1], cen_outlet[3 * i + 2]);
#endif
    ...
    else
    {
        ...
        for (int i = 1; i <= num_inlet; i++)
            mask_value[i] = 1;
        for (int i = 1; i <= num_outlet; i++)
            mask_value[num_inlet + i] = 2;
    }
#ifdef DEBUG
    ...
    {
        if (bc_mask[ele] != 0)
            bc_mask[ele] = mask_value[bc_mask[ele]];
    }
    // creat mesh struct
